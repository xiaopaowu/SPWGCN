...
def load_corpus(dataset_str,image_flag):

    # names = ['tx', 'ty', 'allx', 'ally', 'adj']
    if image_flag:
        names = ['tx', 'ty', 'allx', 'ally', '_image_adj']
    else:
        names = ['tx', 'ty', 'allx', 'ally', 'adj']
    objects = []
    for i in range(len(names)):
        with open("D:/vscode/SPWGCN/data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))

    tx, ty, allx, ally, adj = tuple(objects)
    print(tx.shape, ty.shape, allx.shape, ally.shape)
    ...
    train_idx_orig = parse_index_file(
        "D:/vscode/SPWGCN/data/{}.train.index".format(dataset_str))
    ...
    y_train[train_mask, :] = labels[train_mask, :]
    y_test[test_mask, :] = labels[test_mask, :]
    # adj加上了adj的转置比adj大的对应元素，减去了adj比adj的转置小的对应元素
    # 将非对称的邻接矩阵adj，变为对称矩阵。表现在图上，结果就是将有向图变为无向图
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)

    return adj, features, y_train, y_test, train_mask, test_mask, train_size, test_size
