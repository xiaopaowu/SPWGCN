...
class case:
    def __init__(self,case_id):
        ...
        self.test_case_front = str()
        self.test_case_behind = str()
        self.test_case_description = str()
...
class Model(object):
    def __init__(self, **kwargs):
        allowed_kwargs = {'name', 'logging', 'all_weight'}
        for kwarg in kwargs.keys():
            assert kwarg in allowed_kwargs, 'Invalid keyword argument: ' + kwarg
        name = kwargs.get('name')
        all_weight = kwargs.get('all_weight')
        if not name:
...
class GCN(Model):
    ...
    def _loss(self):
        for var in self.layers[0].vars.values():
            self.loss += FLAGS.weight_decay * tf.nn.l2_loss(var)
        self.loss += weight_masked_softmax_cross_entropy(self.outputs, self.placeholders['labels'], self.placeholders['labels_mask'],self.all_weight)
